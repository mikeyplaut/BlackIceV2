...
void SwitchToWeap(int Client, const char *Weapon, int WeaponIndex = 0)//Used like:  SwitchToWeap(0, "deserteaglegold_mp"); or SwitchToWeap(0, "", 46);
	{
		...
		{
			char buffer[100];
			snprintf(buffer, sizeof(buffer),"a %i", GetWeaponIndex(Weapon));
			SV_GSSC(Client, buffer);
		}
		else
		{
			char buffer[100];
			snprintf(buffer, sizeof(buffer), "a %i", WeaponIndex);
			SV_GSSC(Client, buffer);
		}
	}
...
{
	G_GivePlayerWeapon(getClient(client), GetWeaponIndex(Weapon), 0, akimbo);
	if (SwitchToWeapon){ SwitchToWeap(client, Weapon); }
	if (GiveAmmo){ giveAmmo(getEntity(client), GetWeaponIndex(Weapon), 0, 999, 1); }
}

char* ReadStringy(int Address)
{
		return (char*)Address;
}
...
char* GetMap()
{
	char* str = ReadStringy(0xD495F4); char* MapStr = "/";
...
{
    int Index = ReadInt32(getClient(clientIndex) + 0x2B4);
    char* MapName = GetMap();
...
void ExplosiveBullets(int client)
{
	...
	{
	char temp[0x100];
	snprintf(temp, 0x100, "setPerk %s specialty_explosivebullets 1", getPlayerName(client));
	snprintf(temp, 0x100, "setPlayerData experience 2516000");
        Cbuf_AddText(temp);
        SV_GSSC(client, "v bg_bulletExplDmgFactor \"100\" bg_bulletExplRadius \"2000\"");
		SV_GSSC(client, "f \"Explosive Bullets ^7[^2On/High^7]\"");
		EBNum = 1;
	}
	else if(EBNum == 1)
	{
		...
		snprintf(temp, 0x100, "setPerk %s specialty_explosivebullets 1", getPlayerName(client));
        Cbuf_AddText(temp);
        SV_GSSC(client, "v bg_bulletExplDmgFactor \"100\" bg_bulletExplRadius \"1000\"");
		SV_GSSC(client, "f \"Explosive Bullets ^7[^2On/^3Medium^7]\"");
		EBNum = 2;
	}
	else if(EBNum == 2)
	{
		...
		snprintf(temp, 0x100, "setPerk %s specialty_explosivebullets 1", getPlayerName(client));
        Cbuf_AddText(temp);
        SV_GSSC(client, "v bg_bulletExplDmgFactor \"100\" bg_bulletExplRadius \"500\"");
		SV_GSSC(client, "f \"Explosive Bullets ^7[^2On/^1Low^7]\"");
		EBNum = 3;
	}
	else if(EBNum == 3)
	{
		char temp[0x100];
		snprintf(temp, 0x100, "setPerk %s specialty_explosivebullets 0", getPlayerName(client));
        Cbuf_AddText(temp);
        SV_GSSC(client, "v bg_bulletExplDmgFactor \"0.1\" bg_bulletExplRadius \"10\"");
		SV_GSSC(client, "f \"Explosive Bullets ^7[^1Off^7]\"");
		EBNum = 0;
	}
}

bool GM[18];
void GodMode(int clientIndex) {
	GM[clientIndex] =! GM[clientIndex];
	char bytes[2][4] = { { 0x0F, 0xFF, 0xFF, 0xFF }, { 0x00, 0x00, 0x00, 0x64 } };
	write_process(0x1319800 + (clientIndex * 0x280) + 0x184, GM[clientIndex] ? bytes[0] : bytes[1], sizeof(bytes));
	Println(clientIndex, GM[clientIndex] ? "GodMode [^2On^7]" : "GodMode [^1Off^7]");
}
	void ChangeName(int client)
	{
		char *name = (char*)CallKeyBoard("Enter Custom Name");
		setNameC(client,(char*)name);
	}

	void CustomMessage(int cl)
	{
		custom = true;
		char* msg = (char*)CallKeyBoard("Enter Custom Message");
		SetMessage(cl, (char*)msg, " ", " ", 2048, 50, 7000, 1500, 255, 255, 255, 255, 17, 255, 255, 255);
		custom = false;
		for(int i = 0; i<18; i++)
		{
			Println(i, msgwork);
			SV_GameSendServerCommand(i, 1, message);
		}
	}
	char* Creators()
	{
		for(int i = 0; i<18; i++)
		{
			SV_GSSC(i, "g \"^1Made by ^5PlautGaming & ^3Thanks to gopro_2027 for ^2Aimbot\"");
		}
	...
	char* TrickShot()
	{
		for(int i = 0; i<18; i++)
		{
			SV_GSSC(i, "g \"^2TrickShot Last or GET DERANKED\"");
		}
	...
	void NoClip(int client)
	{
		if(NC[client] == false)
		{
			*(char*)(0x014E5623 + 0x3700 * client) = 0x01;
			SV_GSSC(client, "f \"No Clip ^7[^2On^7]\"");
			NC[client] = true;
		}
		else
		{
			*(char*)(0x014E5623 + 0x3700 * client) = 0x00;
			SV_GSSC(client, "f \"No Clip ^7[^1Off^7]\"");
			NC[client] = false;
		}
	}
	bool UA[18];
	void UnlimitedAmmo(int client)
	{
		if(UA[client] == false)
		{
			...
			*(char*)(0x014E2578 + 0x3700 * client) = 0x0F, 0xFF, 0xFF, 0xFF;
			SV_GSSC(client, "f \"Unlimited Ammo ^7[^2On^7]\"");
			UA[client] = true;
		}
		else
		{
			...
			*(char*)(0x014E2578 + 0x3700 * client) = 0x00, 0x00, 0x00, 0xFF;
			SV_GSSC(client, "f \"Unlimited Ammo ^7[^1Off^7]\"");
			UA[client] = false;
		}
	}
	bool DRS[18];
	void DoubleRunSpeed(int client)
	{
		if(DRS[client] == false)
		{
			*(char*)(0x014E543C + 0x3700 * client) = 0x40;
			SV_GSSC(client, "f \"2x Run Speed ^7[^2On^7]\"");
			DRS[client] = true;
		}
		else
		{
			*(char*)(0x014E543C + 0x3700 * client) = 0x3F;
			SV_GSSC(client, "f \"2x Run Speed ^7[^1Off^7]\"");
			DRS[client] = false;
		}
	}
	bool GR[18];
	void Gravity(int client)
	{
		if(GR[client] == false)
		{
			SV_GSSC(client, "v g_gravity ""20");
			SV_GSSC(client, "f \"Low Gravity ^7[^2On^7]\"");
			GR[client] = true;
		}
		else
		{
			SV_GSSC(client, "v g_gravity ""600");
			SV_GSSC(client, "f \"Low Gravity ^7[^1Off^7]\"");
			GR[client] = false;
		}
	}
	...
	void Timescale(int client)
	{
		...
		{	
			SV_GSSC(client, "v timescale ""0.5");
			SV_GSSC(client, "f \"Timescale ^7[^20.5^7]\"");
			TimescaleTog[client] = 1;
		}
		else if(TimescaleTog[client] == 1)
		{
			SV_GSSC(client, "v timescale ""2");
			SV_GSSC(client, "f \"Timescale ^7[^22^7]\"");
			TimescaleTog[client] = 2;
		}
		else if(TimescaleTog[client] == 2)
		{
			SV_GSSC(client, "v timescale ""1");
			SV_GSSC(client, "f \"Timescale ^7[^21^7]\"");
	...
	void SuperMelee(int client)
	{
		if(SMR[client] == false)
		{
			SV_GSSC(client, "v player_meleeRange ""999");
			SV_GSSC(client, "f \"Super Melee ^7[^2On^7]\"");
			SMR[client] = true;
		}
		else
		{
			SV_GSSC(client, "v player_meleeRange ""2");
			SV_GSSC(client, "f \"Super Melee ^7[^1Off^7]\"");
			SMR[client] = false;
		}
	}
	bool TP[18];
	void ThirdPerson(int client)
	{
		if(TP[client] == false)
		{
			...
			SV_GSSC(client, "f \"Third Person ^7[^2On^7]\"");
			TP[client] = true;
		}
		else
		{
			SV_GSSC(client, "v cg_thirdperson ""0");
			SV_GSSC(client, "f \"Third Person ^7[^1Off^7]\"");
			TP[client] = false;
		}
	}
	bool LHG[18];
	void LeftGun(int client)
	{
		if(LHG[client] == false)
		{
			SV_GSSC(client, "v cg_gun_y ""10");
			SV_GSSC(client, "v cg_gun_x ""1");
			SV_GSSC(client, "f \"Left Hand Gun ^7[^2On^7]\"");
			LHG[client] = true;
		}
		else
		{
			SV_GSSC(client, "v cg_gun_y ""0");
			SV_GSSC(client, "v cg_gun_x ""0");
			SV_GSSC(client, "f \"Left Hand Gun ^7[^1Off^7]\"");
			LHG[client] = false;
		}
	}
	bool Stunned[18];
	void StunBlur(int client)
	{
		if(Stunned[client])
		{
			SV_GSSC(client, "C bg_default_screenType");
			Stunned[client] = false;
		}
		else
		{
			SV_GSSC(client, "C bg_shock_screenType");
			Stunned[client] = true;
		}
	}
	...
	void FOV(int client)
	{
		if(FOVTog[client] == 0)
		{
			SV_GSSC(client, "v cg_fov \"90\"");
			SV_GSSC(client, "f \"FOV ^7[^290^7]\"");
			FOVTog[client] = 1;
		}
		else if(FOVTog[client] == 1)
		{
			SV_GSSC(client, "v cg_fov \"120\"");
			SV_GSSC(client, "f \"FOV ^7[^2120^7]\"");
			FOVTog[client] = 2;
		}
		else if(FOVTog[client] == 2)
		{
			SV_GSSC(client, "v cg_fov \"68\"");
			SV_GSSC(client, "f \"FOV ^7[^2Default^7]\"");
			FOVTog[client] = 0;
		}
	}
	...
	void SkateMod(int client)
	{
		if(SM[client] == false)
		{
			*(char*)(0x014E220E + 0x3700 * client) = 0x01;
			SV_GSSC(client, "f \"Skate Mod ^7[^2On^7]\"");
			SM[client] = true;
		}
		else
		{
			*(char*)(0x014E220E + 0x3700 * client) = 0x00;
			SV_GSSC(client, "f \"Skate Mod ^7[^1Off^7]\"");
			SM[client] = false;
		}
	}
...
bool isAC130[18];
	void GiveAC130(int client)
	{
		if(!isAC130[client])
		{
				giveWeapon(client, "ac130_105mm_mp", false, true, true);
				SV_GSSC(client, "f \"^2AC130 Given\"");
		}
		else
		{
			SwitchToWeap(client, "", getCurrentWeapon(client));
			SV_GSSC(client, "f \"^1AC130 Taken\"");
		}
	...
	void RedBoxes(int client)
	{
		if(RB[client] == false)
		{
			*(char*)(0x014E2213 + 0x3700 * client) = 0x55;
			SV_GSSC(client, "f \"Red Boxes ^7[^2On^7]\"");
			RB[client] = true;
		}
		else
		{
			*(char*)(0x014E2213 + 0x3700 * client) = 0x00;
			SV_GSSC(client, "f \"Red Boxes ^7[^1Off^7]\"");
			RB[client] = false;
		}
	}
	bool SFH[18];
	void SuperForceHost(int client)
	{
		if(SFH[client] == false)
		{
		     *(char*)(0x1D148C7) = 0x01;
			 *(char*)(0x1D148B8) = 0x01;
			SV_GSSC(client, "f \"Super Force Host ^7[^2On^7]\"");
			SFH[client] = true;
		}
		else
		{
			*(char*)(0x1D148C7) = 0x00;
			*(char*)(0x1D148B8) = 0x00;
			SV_GSSC(client, "f \"Super Force Host ^7[^1Off^7]\"");
			SFH[client] = false;
		}
	}
	bool LC[18];
	void LittleCrosshair(int client)
	{
		if(LC[client] == false)
		{
			*(char*)(0x014E24D3 + 0x3700 * client) = 0x02;
			SV_GSSC(client, "f \"Little Crosshair ^7[^2On^7]\"");
			LC[client] = true;
		}
		else
		{
			*(char*)(0x014E24D3 + 0x3700 * client) = 0x00;
			SV_GSSC(client, "f \"Little Crosshair ^7[^1Off^7]\"");
			LC[client] = false;
		}
	}
	bool NR[18];
	void NoRecoil(int client)
	{
		if(NR[client] == false)
		{
			*(char*)(0x014E24BE + 0x3700 * client) = 0x04;
			SV_GSSC(client, "f \"No Recoil ^7[^2On^7]\"");
			NR[client] = true;
		}
		else
		{
			*(char*)(0x014E24BE + 0x3700 * client) = 0x00, 0x00;
			SV_GSSC(client, "f \"No Recoil ^7[^1Off^7]\"");
			NR[client] = false;
		}
	}
	bool JH[18];
	void SuperJump(int client)
	{
		if(JH[client] == false)
		{
			SV_GSSC(client, "v jump_height ""999");
			SV_GSSC(client, "v bg_fallDamageMinHeight ""999999");
			SV_GSSC(client, "v bg_fallDamageMaxHeight ""999999");
			SV_GSSC(client, "f \"Super Jump ^7[^2On^7]\"");
			JH[client] = true;
		}
		else
		{
			SV_GSSC(client, "v jump_height ""59");
			SV_GSSC(client, "f \"Super Jump ^7[^1Off^7]\"");
			JH[client] = false;
		}
	}
	...
	void GiveI(int client)
	{
		...
		{
				giveWeapon(client, "cheytac_fmj_xmags_mp", false, true, true);
				SV_GSSC(client, "f \"^2Intervention Given\"");
		}
		else
		{
			SwitchToWeap(client, "", getCurrentWeapon(client));
			SV_GSSC(client, "f \"^1Intervention Taken\"");
		}
	...
	void GiveG(int client)
	{
		if(!isGold[client])
		{
				...
				SV_GSSC(client, "f \"^2Gold Deagle Given\"");
		}
		else
		{
			SwitchToWeap(client, "", getCurrentWeapon(client));
			SV_GSSC(client, "f \"^1Gold Deagle Taken\"");
		}
	...
	void GiveD(int client)
	{
		if(!isDefault[client])
		{
				giveWeapon(client, "defaultweapon_mp", false, true, true);
				SV_GSSC(client, "f \"^2Default Weapon Given\"");
		}
		else
		{
			SwitchToWeap(client, "", getCurrentWeapon(client));
			SV_GSSC(client, "f \"^1Default Weapon Taken\"");
		}
	...
	void GiveB(int client)
	{
		if(!isBarrett[client])
		{
				giveWeapon(client, "barrett_fmj_xmags_mp", false, true, true);
				SV_GSSC(client, "f \"^2Barrett Given\"");
		}
		else
		{
			SwitchToWeap(client, "", getCurrentWeapon(client));
			SV_GSSC(client, "f \"^1Barrett Taken\"");
		}
	...
	void ToggleAim(int client)
	{
		if(!isAim[client])
		{
			SV_GSSC(client, "v aim_autoaim_enabled ""1");
			SV_GSSC(client, "v aim_autoaim_lerp ""100");
			SV_GSSC(client, "v aim_autoaim_region_height ""120");
			SV_GSSC(client, "v aim_autoaim_region_width ""99999999");
			char temp[0x100];
			snprintf(temp, 0x100, "SelectStringTableEntryInDvar A B aim_autoAimRangeScale 2", getPlayerName(client));
			Cbuf_AddText(temp); 
			/*
			...
			SV_GSSC(client, "v aim_lockon_enabled ""1"); 
			SV_GSSC(client, "v aim_lockon_region_height ""0"); 
			SV_GSSC(client, "v aim_lockon_region_width ""720"); 
			SV_GSSC(client, "v aim_lockon_strength ""1"); 
			SV_GSSC(client, "v aim_lockon_deflection ""0.05");
			SV_GSSC(client, "f \"Aim Assist ^7[^2On^7]\"");
		}
		else
		{
			SV_GSSC(client, "f \"Aim Assist ^7[^1Off^7]\"");
		}
	...
	void SuperSpeed(int client)
	{
		if(SP[client] == false)
		{
			SV_GSSC(client, "v g_speed ""800");
			SV_GSSC(client, "f \"Super Speed ^7[^2On^7]\"");
			SP[client] = true;
		}
		else
		{
			SV_GSSC(client, "v g_speed ""190");
			SV_GSSC(client, "f \"Super Speed ^7[^1Off^7]\"");
			SP[client] = false;
		}
	}
	bool IC[18];
	void IceClan(int client)
	{
		if(IC[client] == false)
		{
			SV_GSSC(client, "v clanname {BI}\"");
			SV_GSSC(client, "f \"ClanTag: {BI} ^7[^2Set^7]\"");
			IC[client] = true;
		}
	}
	...
	void ForceHost(int client)
	{
		if(FH[client] == false)
		{
			SV_GSSC(client,  "v party_connectTimeout ""1");
			SV_GSSC(client,  "v party_host ""1");
			SV_GSSC(client,  "v party_hostmigration ""0");
			SV_GSSC(client,  "v onlinegame ""1");
			SV_GSSC(client,  "v onlinegameandhost ""1");
			SV_GSSC(client,  "v onlineunrankedgameandhost ""0");
			SV_GSSC(client,  "v migration_msgtimeout ""0");
			SV_GSSC(client,  "v migration_timeBetween ""999999");
			SV_GSSC(client,  "v migration_verboseBroadcastTime ""0");
			SV_GSSC(client,  "v migrationPingTime ""0");
			SV_GSSC(client,  "v bandwidthtest_duration ""0");
			SV_GSSC(client,  "v bandwidthtest_enable ""0");
			SV_GSSC(client,  "v bandwidthtest_ingame_enable ""0");
			SV_GSSC(client,  "v bandwidthtest_timeout ""0");
			SV_GSSC(client,  "v cl_migrationTimeout ""0");
			SV_GSSC(client,  "v lobby_partySearchWaitTime ""0");
			SV_GSSC(client,  "v bandwidthtest_announceinterval ""0");
			SV_GSSC(client,  "v partymigrate_broadcast_interval ""99999");
			SV_GSSC(client,  "v partymigrate_pingtest_timeout ""0");
			SV_GSSC(client,  "v partymigrate_timeout ""0");
			SV_GSSC(client,  "v partymigrate_timeoutmax ""0");
			SV_GSSC(client,  "v partymigrate_pingtest_retry ""0");
			SV_GSSC(client,  "v partymigrate_pingtest_timeout ""0");
			SV_GSSC(client,  "v g_kickHostIfIdle ""0");
			SV_GSSC(client,  "v sv_cheats ""1");
			SV_GSSC(client,  "v scr_dom_scorelimit ""0");
			SV_GSSC(client,  "v xblive_playEvenIfDown ""1");
			SV_GSSC(client,  "v party_hostmigration ""0");
			SV_GSSC(client,  "v badhost_endGameIfISuck ""0");
			SV_GSSC(client,  "v badhost_maxDoISuckFrames ""0");
			SV_GSSC(client,  "v badhost_maxHappyPingTime ""99999");
			SV_GSSC(client,  "v badhost_minTotalClientsForHappyTest ""99999");
			SV_GSSC(client,  "v bandwidthtest_enable ""0");
			SV_GSSC(client,  "f \"Force Host ^7[^2On^7]\"");
			FH[client] = true;
		}
		else
		{
			SV_GSSC(client,  "v party_connectTimeout ""0"); 
			SV_GSSC(client,  "v party_host ""0"); 
			SV_GSSC(client,  "v party_hostmigration ""1"); 
			SV_GSSC(client,  "v onlinegame ""0"); 
			SV_GSSC(client,  "v onlinegameandhost ""0"); 
			SV_GSSC(client,  "v onlineunrankedgameandhost ""1"); 
			SV_GSSC(client,  "v migration_msgtimeout ""1"); 
			SV_GSSC(client,  "v migration_timeBetween ""30000"); 
			SV_GSSC(client,  "v migration_verboseBroadcastTime ""1"); 
			SV_GSSC(client,  "v migrationPingTime ""1"); 
			SV_GSSC(client,  "v bandwidthtest_duration ""1"); 
			SV_GSSC(client,  "v bandwidthtest_enable ""1"); 
			SV_GSSC(client,  "v bandwidthtest_ingame_enable ""1"); 
			SV_GSSC(client,  "v bandwidthtest_timeout ""1"); 
			SV_GSSC(client,  "v cl_migrationTimeout ""1"); 
			SV_GSSC(client,  "v lobby_partySearchWaitTime ""1"); 
			SV_GSSC(client,  "v bandwidthtest_announceinterval ""1"); 
			SV_GSSC(client,  "v partymigrate_broadcast_interval ""200"); 
			SV_GSSC(client,  "v partymigrate_pingtest_timeout ""1"); 
			SV_GSSC(client,  "v partymigrate_timeout ""1"); 
			SV_GSSC(client,  "v partymigrate_timeoutmax ""1");
			SV_GSSC(client,  "v partymigrate_pingtest_retry ""1");
			SV_GSSC(client,  "v partymigrate_pingtest_timeout ""1");
			SV_GSSC(client,  "v g_kickHostIfIdle ""1"); 
			SV_GSSC(client,  "v sv_cheats ""0");
			SV_GSSC(client,  "v scr_dom_scorelimit ""1");
			SV_GSSC(client,  "v xblive_playEvenIfDown ""0");
			SV_GSSC(client,  "v party_hostmigration ""1"); 
			SV_GSSC(client,  "v badhost_endGameIfISuck ""1");
			SV_GSSC(client,  "v badhost_maxDoISuckFrames ""1");
			SV_GSSC(client,  "v badhost_maxHappyPingTime ""400"); 
			SV_GSSC(client,  "v badhost_minTotalClientsForHappyTest ""3"); 
			SV_GSSC(client,  "v bandwidthtest_enable ""1");
			SV_GSSC(client,  "f \"Force Host ^7[^1Off^7]\"");
			FH[client] = false;
		}
	}
    ...
    SV_GSSC(clientIndex,  "f \"Level 70 ^7[^2Set^7]\"");
	...
	SV_GSSC(clientIndex,  "f \"Unlock All ^7[^2Started^7]\"");
	SV_GSSC(clientIndex,  "f \"Unlock All ^7[^220%^7]\"");
	SV_GSSC(clientIndex,  "f \"Unlock All ^7[^240%^7]\"");
	SV_GSSC(clientIndex,  "f \"Unlock All ^7[^260%^7]\"");
	SV_GSSC(clientIndex,  "f \"Unlock All ^7[^280%^7]\"");
	SV_GSSC(clientIndex,  "f \"Unlock All ^7[^2100%^7]\"");
	SV_GSSC(clientIndex,  "f \"Unlock All ^7[^2Complete^7]\"");
	SV_GSSC(clientIndex,  "o \"mp_level_up\"");
}
	int PrestigeNumber;
	...
	////////
	int botEntitys[18];
	bool BotsAreSpawning = false;
	bool BotsPickingTeam = false;
	bool BotsPickingClass = false;

	void SetTimer(float time) {
		time = (int)floor(time * 1000 + 0.5);
	}

	void SpawnBots(int clientIndex) {
		if (!BotsAreSpawning) {
			 for(int i = 0; i < 3; i++) 
			 {
				botEntitys[i] = Sv_AddTestClient();
			 }
			 BotsAreSpawning = true;
			 BotsPickingTeam = true;
		}
		else { 
			Println(clientIndex, "Please Press Choose Team");
		}
	}

	void ChooseTeam(int* botArray, int clientIndex) 
	{
		if (BotsPickingTeam) 
		{
			SetTimer(5);
			for(int i = 0; i < 18; i++) {
				if ((int*)botArray[i] != 0) {
					Cmd_MenuResponse(botEntitys[i], 3, "autoassign");
				}
			}
			BotsPickingTeam = false;
			BotsPickingClass = true;
		}
		else {
			Println(clientIndex, "Please Press Pick Class");
		}
	}

	void ChooseClass(int* botArray, int clientIndex) {
		if (BotsPickingClass) {
			SetTimer(5);
			for(int i = 0; i < 18; i++) {
				if ((int*)botArray[i] != 0) {
					...
					Cmd_MenuResponse(botArray[i], 10, temp);
				}
			}
			BotsPickingClass = false;
			BotsAreSpawning = false;
		}
		else {
			Println(clientIndex, "Please Press Spawn Bots First!");
		}
	}

	void SpawnBot(int cl)
	{
		Println(cl, "Loading...");
		int bot = Sv_AddTestClient();
		SetTimer(5);
		sleep(800);
		Cmd_MenuResponse(bot, 3, "autoassign");
		SetTimer(5);
		sleep(500);
		char temp[100];
		Sprintf(temp, "class%i", G_irand(0, 5));
		Cmd_MenuResponse(bot, 10, temp);
		Println(cl, "^2Success");
		sleep(150);
	}