...
bool UnfairAimbot = false;
bool silentAimbot = false;//not really used YET

//nonhost button monitoring
enum Buttons
{
    L1 = 0x41,
    R1 = 0x4D,
...
bool isAiming() {
	return (pressedButton(L2) && pressedButton(R2) || pressedButton(L2)) || (shootToEnable && pressedButton(R2)) || *(char*)0x0095afe0 == 1 || *(char*)*(int*)0x9FD590 == 1;
}
int GetHealth()
{
	int cg_s = *(int*)(0x915254);
...
int wp_redo(int r3) {
	...
	write_process(0x000A1A88,bytes,16);//crashes here
...
int clcnc_redo(int r3) {
	...
	write_process(0x000A3BB0,bytes,16);
